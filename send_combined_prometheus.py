...
run_id_metric = Gauge('run_id_metric', 'Run ID metric', ['src_ip', 'dst_ip', 'run_id'])
...
def parse_and_collect_send_output():
    process = subprocess.Popen(['./send', '--interval', '100000', '--iface', 'eno1d1', '--dst-ip=10.10.1.2'], stdout=subprocess.PIPE, text=True)
    src_ip = None
    dst_ip = None
    send_interval = None
    start_offset = None
        ...
        if 'sending' in line:
            ...
            if match:
                ...
                if run_id is None and send_interval is not None:
                    ...
                    run_id_metric.labels(src_ip, dst_ip, run_id).set(float(run_id))
                send_time_metric.labels(src_ip, dst_ip, run_id).set(send_time)
